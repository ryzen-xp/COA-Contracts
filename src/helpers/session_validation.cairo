...
pub mod SessionValidationMiddleware {
    ...
    #[constructor]
    fn constructor(ref self: ContractState, session_system_address: ContractAddress) {
        // Store the session system address for future dispatcher calls
    ...
    impl SessionValidationMiddlewareImpl of ISessionValidation<ContractState> {
        ...
        fn require_valid_session_for_player(
            ...
            if !is_valid {
                // Emit failure event
                self.emit(Event::SessionValidationFailed(
                    SessionValidationFailed {
                        session_id,
                        player,
                        error_code: super::ERROR_INVALID_SESSION,
                    }
                ));
            ...
            // Emit success event
            self.emit(Event::SessionValidationSuccess(
        ...
        fn increment_session_transaction_count(
            ...
            self.emit(Event::SessionTransactionIncremented(
                SessionTransactionIncremented {
                    session_id,
                    player,
                    new_count: 1, // Placeholder
                }
            ));
        }
    }
