...
/// 1. `stats`: Calculates and returns the specific statistics for an armour type (`Armour`) using a
/// `match` pattern.
/// 2. `index`: Returns a unique index (`u8`) to identify the armour type, also using a `match`
/// pattern.
...
///
/// Implements the `Into<Armour, ByteArray>` trait to convert an `Armour` into its string
/// representation.
