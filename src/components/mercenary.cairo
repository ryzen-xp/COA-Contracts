...
fn generate_traits(seed: u256) -> Traits {
    ...
    Traits { armour: armour.into(), weapon: weapon.into() }
}
...
/// 1. `new`:Creates a new `Mercenary` with the specified `id`, `owner`, generated `traits`, and
/// calculated `stats` from the `seed`.
///
#[generate_trait]
impl MercenaryImpl of MercenaryTrait {
    fn new(owner: ContractAddress, id: u128, seed: u256) -> Mercenary {
        ...
        return Mercenary { id, owner, traits, stats };
    }
}
...
///
/// The implementation of `Display` for types that implement `Into` and `Copy` allows formatting a
/// type as a string.
/// 1. `fmt`: Formats the type by converting it into a `ByteArray` and appending it to the formatter
/// buffer.
///
