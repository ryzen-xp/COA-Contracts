...
/// 1.`seed`: Computes a Pedersen hash of the transaction hash and the contract address (`salt`) to
/// generate a seed.
///
...
/// 1. `new`: Initialize `Random` struct with a new seed derived from the contract address and sets
/// the nonce to 0.
/// 2. `next_seed`: Generates a new seed by applying the Pedersen hash using the current seed and
/// nonce.
...
///
/// `get_uuid`: Hashes the transaction hash and world UUID using Poseidon and returns the resulting
/// UUID as a `u128`.
///
