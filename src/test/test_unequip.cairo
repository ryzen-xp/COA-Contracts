...
mod tests {
    ...
    fn test_unequip_preserves_other_items() {
        ...
        assert(
            body.upper_torso.len() == original_chest.len(), 
            'Chest armor should be preserved'
        );
        assert!(
        ...
        assert!(
            body.waist.len() == original_waist.len(), 
            "Waist items count should be preserved"
        );
        assert!(
            array_contains(body.waist, FIREARM_ID), 
            "Firearm should be preserved in waist"
        );
        assert(
            body.vehicle == original_vehicle, 
            'Vehicle should be preserved'
        );
        assert(
            body.back == original_back, 
            'Back item should be preserved'
        );
    }
