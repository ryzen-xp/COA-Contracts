...
mod tests {
    ...
    fn test_new_cooldown_initial_state() {
        let cooldown = Cooldown { player_id: 1, action_type: 'jump', ready_at: 0 };
    ...
    fn test_set_cooldown_sets_ready_at_correctly() {
        let mut cooldown = Cooldown { player_id: 1, action_type: 'dash', ready_at: 0 };
        ...
        assert(cooldown.ready_at == fake_time + delay, 'set current_time + delay');
    }

    #[test]
    #[should_panic(expected: 'Invalid_delay')]
    fn test_set_cooldown_with_zero_delay_panics() {
        let mut cooldown = Cooldown { player_id: 2, action_type: 'attack', ready_at: 0 };
    ...
    fn test_multiple_cooldowns_different_players() {
        let mut c1 = Cooldown { player_id: 1, action_type: 'jump', ready_at: 0 };

        let mut c2 = Cooldown { player_id: 2, action_type: 'jump', ready_at: 0 };
